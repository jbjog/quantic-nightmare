...
public class StageMenuHud extends QNMenuHud {
    ...
    public StageMenuHud(Gamer screen) {
        ...
        stagesTable = new FocusableTable(TextValues.CHOOSE[QNPreferences.getPref().getLanguage().ordinal()]);
                ...
                public void clicked(InputEvent event, float x, float y) {
                    try {
                        ...
                        if(e instanceof IllegalLevelInsertionException){
                            messageTable.getTitleLabel().setText(TextValues.LEVEL_BROKEN[QNPreferences.getPref().getLanguage().ordinal()]);
                        }else if(e instanceof UnknownLevelException){
                            messageTable.getTitleLabel().setText(TextValues.LEVEL_NOT_AVAILABLE[QNPreferences.getPref().getLanguage().ordinal()]);
                        }
        ...
        stagesTable.addLabel(TextValues.BACK[QNPreferences.getPref().getLanguage().ordinal()],new ClickListener() {
            @Override
            public void clicked(InputEvent event, float x, float y) {
                escaped();
            }
    ...
    private static String getLevelImagePath(int levelNumber){
        ...
        PlayerScoreDTO ps = FileHandling.readScore(levelNumber);
